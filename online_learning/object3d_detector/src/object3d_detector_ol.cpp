...
Object3dDetector::Object3dDetector() {
  ...
  private_nh.param<float>("z_limit_min", z_limit_min_, -0.6);
  ...
  /*** Subscribers ***/
  point_cloud_sub_ = node_handle_.subscribe<sensor_msgs::PointCloud2>("/os_cloud_node/points", 100, &Object3dDetector::pointCloudCallback, this);
...
/*         ROS_INFO("centroid: X= %f Y= %f Z= %f", f.centroid[0], f.centroid[1], f.centroid[2]);
      ...
      ROS_INFO("fectures vector: %lu", features_.size());
      ROS_INFO("clusters vector: %lu", learnable_clusters_.size());
    }
  }
}
  ...
  for(std::vector<Feature>::iterator it = features_.begin(); it != features_.end(); it++) {
    ...
    if(train_round_ > 0) {
      ...
      if(svm_check_probability_model(svm_model_)) {
        ...
        ROS_INFO("probability: %f", prob_estimates[0]);
        if(prob_estimates[0] > human_probability_)
          svm_find_human = true;
      } else {
        double svm_p = svm_predict(svm_model_, svm_node_);
        ROS_INFO("svm_p: %f", svm_p);
        if(svm_p == 1)
          svm_find_human = true;
      }
    }
